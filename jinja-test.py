        ...
        for function_block in function_blocks:
            for arg_key, argument in function_block.arguments.items():
                if remove_ref_symbols(function_template[function].arguments.get(arg_key)[0].arg_type) in type_defs.values():
                    function_template[function].arguments.get(arg_key)[0].arg_type = argument[0].arg_type
                        ...
                        for argument_value in argument[0].potential_outputs:
                            if not contains_usage(argument_value, usage_seen[function][arg_key]):
                                ...
                                usage_seen[function][arg_key].update(
                                    get_stripped_usage(argument_value))
                    elif not contains_usage(argument[0].usage, usage_seen[function][arg_key]):
                        pre_processed_data[function].arguments.setdefault(
                            arg_key, []).append(argument[0])
                        current_args_dict[function].append(arg_key)
                        # Update the set with the new arg.usage value
                        usage_seen[function][arg_key].update(
                            get_stripped_usage(argument[0].usage))
...
def generate_code(function_dict: Dict[str, FunctionBlock],
                  ...
                  aliases: Dict[str, str],
                  harness_folder):
    code = code_gen.harness_generator(
...
def generate_includes(all_includes: List[str], harness_folder: str):
    code = code_gen.harness_includes(all_includes)
    code_gen.write_to_file(f'{harness_folder}/FirnessIncludes.h', code)
...
def collect_all_function_arguments(input_data: Dict[str, List[FunctionBlock]],
    ...
    pre_processed_data = variable_fuzzable(
        input_data, types, pre_processed_data, aliases)
...
def analyze_generators(input_generators: Dict[str, List[FunctionBlock]],
    ...
    generators = variable_fuzzable(
        input_generators, types, generators, aliases)
...
def main():
    ...
    data, function_template = load_data(
        args.data_file, harness_functions, macros_val)
