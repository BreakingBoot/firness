...
def harness_generator(functions: Dict[str, SmiInfo], 
    ...
    for function, smi_info in functions.items():
        ...
        output.append(f"    EFI_SMM_COMMUNICATION_PROTOCOL *SmmCommunication;")
        output.append(f'    EDKII_PI_SMM_COMMUNICATION_REGION_TABLE           *PiSmmCommunicationRegionTable;')
        output.append(f'    UINTN MinimalSizeNeeded = EFI_PAGE_SIZE;')
        output.append(f'    UINTN   CommSize;')
        output.append(f'    UINT8    *CommBuffer;')
        output.append(f'    EFI_SMM_COMMUNICATE_HEADER   *CommHeader;')
        output.append(f'    UINT32      Index;')
        output.append(f'    EFI_MEMORY_DESCRIPTOR   *Entry;')
        output.append(f'    UINTN  Size;')
        output.append("")
